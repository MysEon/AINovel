...
const FirstCharacterOnboardingModal = ({
  ...
  useEffect(() => {
    if (!open || templateRegistryProp) return;
  ...
  }, [open, activeTab, loadCharacterGenerationModels]);
  ...
  const renderManualTab = () => (
    <div className="first-character-manual-panel">
      ...
      <div className="first-character-manual-actions">
        <Button type="primary" loading={savingManual} onClick={handleManualCreate}>
          创建第一个角色
        </Button>
      </div>
    </div>
  ...
  const renderAiTab = () => {
    ...
    return (
      <div className="first-character-ai-panel">
        <Form form={aiForm} layout="vertical" className="first-character-ai-form">
          <Form.Item
          ...
          >
            <TextArea
              ...
              showCount
              maxLength={2000}
              placeholder="用一段话描述你的主角，比如：一个内向的图书管理员，秘密拥有读心术能力，渴望被看见但又害怕被理解…"
            />
          </Form.Item>
        </Form>
